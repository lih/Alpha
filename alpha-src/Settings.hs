...
module Settings (Action(..),Settings(..),helpMsg,getSettings,putSettings,withSettings) where
...
options = map (\(Option a b c d) -> Option a b c (intercalate "\n" $ wrap words unwords 80 d))
  ...
  ,Option ['D'] ["dest-dir"] (ReqArg DestDir "<dir>")
   "writes the executables into <dir> (defaults to the current directory)"
  ,sep
  ...
  where sep = Option [] [] undefined "-----------------"
        ...
        str2fmt _s = case splitArg _s of
          ["raw",n] -> raw (read n)
          [s] -> fromMaybe err $ lookup s [(formatName f,f) | f <- formats]
          _ -> err
          where err = error $ "Invalid format argument "++show _s
...
getSettings args = case getOpt Permute options args of
  ...
  where handleOpt s Help             = s { action = PrintHelp }
        ...
        handleOpt s (DestDir d)      = s { destDir = d }
        handleOpt s (Architecture a) = s { outputArch = a }
        handleOpt s (Format f)       = s { outputFmt = f }

settingsRef = unsafePerformIO $ newIORef (undefined :: Settings)
putSettings = writeIORef settingsRef
withSettings :: ((?settings :: Settings) => IO a) -> IO a
withSettings m = readIORef settingsRef >>= \s -> let ?settings = s in m
